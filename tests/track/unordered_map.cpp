...
SUITE(unordered_map_tests)
{
	using namespace rsl;
	using namespace rsl::track;

	TEST(find_unordered_map_test)
	{
		unordered_map<int, int> a{ { 0, 1 },{ 1, 2 },{ 2, 3 } };
	...
	TEST(iterator_unordered_map_test)
	{
	...
	TEST(delete_unordered_map_test)
	{
		auto a = std::make_unique<unordered_map<int, char>>();
		*a = { { 0, 'a' },{ 1, 'b' },{ 2, 'c' } };
	...
	TEST(clear_unordered_map_test)
	{
		unordered_map<char, float> a{ { 'a', 3.f },{ 'b', 5.f },{ 'c', 0.2f } };
	...
	TEST(erase_unordered_map_test)
	{
		unordered_map<int, std::pair<int, std::string>> a{ { 0,{ 1, "hello" } },{ 1,{ 2, "world" } },{ 2,{ 3, "!" } } };
	...
	TEST(insert_unordered_map_test)
	{
	...
	TEST(complex_object_unordered_map_test)
	{
		...
		{
			unordered_map<int, ComplexObject> a{ { 0, ComplexObject() },{ 2, ComplexObject() },{ 4, ComplexObject() } };
